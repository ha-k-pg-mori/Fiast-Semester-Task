...
// 定数定義
// ==============================
const int DIGITS = 4;                       //※※　DIGITSという名前の定数or定義で値は４	// 問題の桁数

// ==============================
// グローバル変数
// ==============================
int count;                              //※※　整数型の変数count					    // 推測回数	
int cursor;                             //※※　整数型の変数cursor					    // 選択カーソル
int target[DIGITS];                         //※※　整数型、配列の要素数DIGITSのtarget	    // 目標数の保存配列
int num[DIGITS];                            //※※　整数型、配列の要素数DIGITSのnum	        // 推測数の保存配列
...
int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nCmdShow)
{
	...
	// ローカル変数宣言
	// ----------------------------------------------------
	int hit = 0;                                       //※※　整数型の変数hitを宣言し、0で初期化
	int blow = 0;                                      //※※　整数型の変数blowを宣言し、0で初期化
	bool gameClear = false;                            //※※　bool型の変数gameClearを宣言し、falseで初期化(trueでゲームクリア)
	...
	int count = 0;                     //※※　// countを０で初期化
	int cursor = 0;                    //※※　// cursorを０で初期化
	...
	while (ProcessMessage() == 0 && CheckHitKey(KEY_INPUT_ESCAPE) == 0)
	{
		...
		if( gameClear == false )
		{
			...
			else if(IsPushKey(MY_INPUT_RIGHT))
			{
				...
				if (DIGITS-1 < cursor)    // 増やした結果、(DIGITS - 1) より大きくなった場合は、(DIGITS - 1)にする
				{
					cursor = DIGITS - 1;
				}
			}
...
void CreateTargetNumber()
{
	srand((unsigned)time(NULL));// 乱数をtime()関数で初期化
...
bool IsValidDigit( int digit )
{
	...
	return true; //※※ // for文を抜けたら重複がないということなので true を返す
}
